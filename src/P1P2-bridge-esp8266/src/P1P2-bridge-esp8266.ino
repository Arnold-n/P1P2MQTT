 ...
 * AsyncMqttClient 0.9.0 by Marvin Roger obtained with git clone or as ZIP from
 * https://github.com/marvinroger/async-mqtt-client ESPAsyncTCP 2.0.1 by Phil Bowles obtained with git clone or as ZIP
 * from https://github.com/philbowles/ESPAsyncTCP/ ESP_Telnet 1.3.1 by  Lennart Hennigs (installed using Arduino IDE)
 *
 * Version history
 ...
 * 20220907 v0.9.21 outputMode/outputFilter status survive ESP reboot (EEPROM), added
 * MQTT_INPUT_BINDATA/MQTT_INPUT_HEXDATA (see P1P2_Config.h), reduced uptime update MQTT traffic 20220904 v0.9.20 added
 * E_SERIES/F_SERIES defines, and F-series VRV reporting in MQTT for 2 models 20220903 v0.9.19 longer MQTT
 * user/password, ESP reboot reason (define REBOOT_REASON) added in reporting 20220829 v0.9.18 state_class added in MQTT
 * discovery enabling visibility in HA energy overview 20220817 v0.9.17 handling telnet welcomestring, error/scopemode
 * time info via P1P2/R/#, v9=ignoreserial 20220808 v0.9.15 extended verbosity command, unique OTA hostname, minor fixes
 ...
 *
 * This program is designed to run on an ESP8266 and bridge between telnet and MQTT (wifi) and P1P2Monitor's serial
 * in/output Additionally, it can reset and program the ATmega328P if corresponding connections are available
 */
...
#ifdef ETHERNET
// Connections W5500: Arduino Mega pins 50 (MISO), 51 (MOSI), 52 (SCK), and 10 (SS)
// (https://www.arduino.cc/en/Reference/Ethernet)
//                       Pin 53 (hardware SS) is not used but must be kept as output. In addition, connect RST, GND, and
//                       5V.
...
#define Sprint_P(pubSerial, pubMqtt, pubTelnet, ...)                                                                   \
  {                                                                                                                    \
    ...
    if (pubSerial)                                                                                                     \
      client_publish_serial(mqttSignal, sprint_value);                                                                 \
    if (pubMqtt)                                                                                                       \
      client_publish_mqtt(mqttSignal, sprint_value);                                                                   \
    if (pubTelnet)                                                                                                     \
      client_publish_telnet(mqttSignal, sprint_value);                                                                 \
  };
...
#endif
#ifdef EKH_SERIES
#include "P1P2_Daikin_ParameterConversion_EKH.h"
#endif
...
void handleCommand(char *cmdString) {
  ...
  case 'a': // reset ATmega
  case 'A':
    Sprint_P(true, true, true, PSTR("* [ESP] Hard resetting ATmega..."));
  ...
  case 'B':
    if ((n = sscanf((const char *)(cmdString + 1), "%19s %i %80s %80s %i %i", &EEPROM_state.EEPROMnew.mqttServer,
                    &EEPROM_state.EEPROMnew.mqttPort, &EEPROM_state.EEPROMnew.mqttUser,
                    &EEPROM_state.EEPROMnew.mqttPassword, &mqttInputByte4, &hwID, &noWiFi)) > 0) {
      Sprint_P(true, true, true, PSTR("* [ESP] Writing new MQTT settings to EEPROM"));
      if (n > 4)
        EEPROM_state.EEPROMnew.mqttInputByte4 = mqttInputByte4;
      if (n > 5)
    ...
    Sprint_P(true, true, true, PSTR("* [ESP] Local IP address: %i.%i.%i.%i"), local_ip[0], local_ip[1], local_ip[2],
             local_ip[3]);
    // TODO enable in future WiFiManager version: if (WiFi.isConnected()) Sprint_P(true, true, true, PSTR("* [ESP]
    // Connected to WiFi SSID %s"), wifiManager.getWiFiSSID()); Sprint_P(true, true, true, PSTR("* [ESP] Pass of AP is
    // %s"), wifiManager.getWiFiPass());
    if (n > 0) {
    ...
    if (n > 0) {
      ...
      mqttClient.setCredentials((EEPROM_state.EEPROMnew.mqttUser[0] == '\0') ? 0 : EEPROM_state.EEPROMnew.mqttUser,
                                (EEPROM_state.EEPROMnew.mqttPassword[0] == '\0') ? 0
                                                                                 : EEPROM_state.EEPROMnew.mqttPassword);
  ...
  case 'd': // reset ESP
  case 'D':
    if (sscanf((const char *)(cmdString + 1), "%d", &temp) == 1) {
      if (temp > 2)
        temp = 2;
      switch (temp) {
      case 0:
        Sprint_P(true, true, true, PSTR("* [ESP] Restarting ESP..."));
#ifdef REBOOT_REASON
        ...
        break;
      case 1:
        Sprint_P(true, true, true, PSTR("* [ESP] Resetting ESP..."));
#ifdef REBOOT_REASON
        ...
        break;
      case 2:
        Sprint_P(true, true, true, PSTR("* [ESP] Resetting maxLoopTime"));
        maxLoopTime = 0;
        break;
      default:
        break;
  ...
  case 'I':
    if ((n = sscanf((const char *)(cmdString + 1), "%i %15s %15s %15s %15s", &useStaticIP,
                    &EEPROM_state.EEPROMnew.static_ip, &EEPROM_state.EEPROMnew.static_gw,
                    &EEPROM_state.EEPROMnew.static_nm)) > 0) {
  ...
  case 'j': // OutputMode
  case 'J':
    if (sscanf((const char *)(cmdString + 1), "%x", &temp) == 1) {
      outputMode = temp;
      ...
      Sprint_P(true, true, true,
               PSTR("* [ESP] %ix 0x0001 to output raw packet data (including pseudo-packets) over mqtt P1P2/R/xxx"),
               outputMode & 0x01);
      Sprint_P(true, true, true,
               PSTR("* [ESP] %ix 0x0002 to output mqtt individual parameter data over mqtt P1P2/P/xxx/#"),
               (outputMode >> 1) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0004 to output json data over mqtt P1P2/J/xxx"),
               (outputMode >> 2) & 0x01);
      Sprint_P(true, true, true,
               PSTR("* [ESP] %ix 0x0008 to have mqtt/json include parameters even if functionality is unknown, "
                    "warning: easily overloads ATmega/ESP (best to combine this with outputfilter >=1)"),
               (outputMode >> 3) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0010 ESP to output raw data over telnet"),
               (outputMode >> 4) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0020 to output mqtt individual parameter data over telnet"),
               (outputMode >> 5) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0040 to output json data over telnet"), (outputMode >> 6) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0080 (reserved for adding time string in R output)"),
               (outputMode >> 7) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0100 ESP to output raw data over serial"),
               (outputMode >> 8) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0200 to output mqtt individual parameter data over serial"),
               (outputMode >> 9) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0400 to output json data over serial"), (outputMode >> 10) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x0800 to output raw bin data over P1P2/X/xxx"),
               (outputMode >> 11) & 0x01);
      Sprint_P(true, true, true,
               PSTR("* [ESP] %ix 0x1000 to output timing data also over P1P2/R/xxx (prefix: C) and via telnet"),
               (outputMode >> 12) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x2000 to output error data also over P1P2/R/xxx (prefix: *)"),
               (outputMode >> 13) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x4000 to use P1P2/R/xxx as input (requires MQTT_INPUT_HEXDATA)"),
               (outputMode >> 14) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x8000 to use P1P2/X/xxx as input (requires MQTT_INPUT_BINDATA)"),
               (outputMode >> 15) & 0x01);
      Sprint_P(true, true, true, PSTR("* [ESP] %ix 0x10000 to include non-HACONFIG parameters in P1P2/P/# "),
               (outputMode >> 16) & 0x01);
    }
    break;
  case 's': // OutputFilter
  case 'S':
    ...
    switch (outputFilter) {
    case 0:
      Sprint_P(true, true, true, PSTR("* [ESP] Outputfilter %s0 all parameters, no filter"),
               (temp != 99) ? "set to " : "");
      break;
    case 1:
      Sprint_P(true, true, true, PSTR("* [ESP] Outputfilter %s1 only changed data"), (temp != 99) ? "set to " : "");
      break;
    case 2:
      Sprint_P(true, true, true, PSTR("* [ESP] Outputfilter %s2 only changed data, excluding temp/flow"),
               (temp != 99) ? "set to " : "");
      break;
    case 3:
      Sprint_P(true, true, true, PSTR("* [ESP] Outputfilter %s3 only changed data, excluding temp/flow/time"),
               (temp != 99) ? "set to " : "");
      break;
    default:
      Sprint_P(true, true, true, PSTR("* [ESP] Outputfilter illegal state %i"), outputFilter);
      break;
    }
    break;
  case '\0':
  ...
  case 'H': // commands v/g/h handled both by P1P2-bridge-esp8266 and P1P2Moniotr
    ...
    case 'V':
      ...
      if (sscanf((const char *)(cmdString + 1), "%2x", &temp) == 1) {
        if (temp < 2) {
          Sprint_P(true, true, true,
                   PSTR("* [ESP] Warning: verbose < 2 or >4 not supported by P1P2-bridge-esp8266, changing to "
                        "verbosity mode 3"));
          cmdString[1] = '3';
          cmdString[2] = '\n';
        }
    ...
    // 'c' 'C' 'p' 'P' 'e' 'E' 't' 'T" 'o' 'O" 'u' 'U" 'x' 'X" 'w' 'W" 'k' 'K' 'l' 'L' 'q' 'Q' 'm' 'M' 'z' 'Z' 'r' 'R'
    // 'n' 'N' 'y' 'Y' handled by P1P2Monitor (reserved 'f' 'F")
  default:
    Sprint_P(true, true, true, PSTR("* [ESP] To ATmega: ->%s<-"), cmdString);
    Serial.print(F(SERIAL_MAGICSTRING));
...
int16_t MQTT_readBuffer_readChar(void) {
  ...
  if (MQTT_readBufferT == MQTT_readBufferH)
    return -1;
  char c = MQTT_readBuffer[MQTT_readBufferT];
  if (++MQTT_readBufferT >= MQTT_RB)
    MQTT_readBufferT = 0;
  return c;
}
#endif
  ...
  if (!len) {
...
#ifdef MQTT_INPUT_BINDATA
  } else if ((!strcmp(topic, mqttInputBinData)) ||
             (!mqttInputBinData[MQTT_KEY_PREFIXIP - 1]) && !strncmp(topic, mqttInputBinData, MQTT_KEY_PREFIXIP - 1)) {
    if (index == 0)
      MQTT_drop = false;
    if ((outputMode & 0x8000) && mqttSetupReady) {
      uint16_t MQTT_readBufferH_new = MQTT_readBufferH + 3 + (total << 1); // 2 for "R ", 2 per byte, 1 for '\n'
      if (((MQTT_readBufferH_new >= MQTT_RB) &&
           ((MQTT_readBufferT > MQTT_readBufferH) || (MQTT_readBufferT <= (MQTT_readBufferH_new - MQTT_RB)))) ||
          ((MQTT_readBufferH_new < MQTT_RB) && (MQTT_readBufferT > MQTT_readBufferH) &&
           (MQTT_readBufferT <= MQTT_readBufferH_new))) {
        // buffer overrun
        Sprint_P(true, false, false,
                 PSTR("* [MON2] Mqtt packet input buffer overrun, dropped, index %i len %i total %i"), index, len,
                 total);
        MQTT_drop = true;
      } else if (!MQTT_drop) {
        if (index == 0) {
        ...
        for (uint16_t i = 0; i < len; i++)
          MQTT_readBuffer_writeHex(payload[i]);
        if (index + len == total)
          MQTT_readBuffer_writeChar('\n');
      }
    } else {
...
#ifdef MQTT_INPUT_HEXDATA
  } else if ((!strcmp(topic, mqttInputHexData)) ||
             (!mqttInputHexData[MQTT_KEY_PREFIXIP - 1]) && !strncmp(topic, mqttInputHexData, MQTT_KEY_PREFIXIP - 1)) {
    if (index == 0)
      MQTT_drop = false;
    if ((outputMode & 0x4000) && mqttSetupReady) {
      ...
      if (((MQTT_readBufferH_new >= MQTT_RB) &&
        ...
        for (uint16_t i = 0; i < len; i++)
          MQTT_readBuffer_writeChar(payload[i]);
        if (index + len == total)
          MQTT_readBuffer_writeChar('\n');
      }
    } else {
...
void setup() {
  ...
  if (!strcmp(EEPROM_state.EEPROMold.signature, EEPROM_SIGNATURE_OLD1)) {
    ...
    strlcpy(EEPROM_state.EEPROMnew.mqttServer, EEPROM_state.EEPROMold.mqttServer,
            sizeof(EEPROM_state.EEPROMnew.mqttServer));
    strlcpy(EEPROM_state.EEPROMnew.mqttPassword, EEPROM_state.EEPROMold.mqttPassword,
            sizeof(EEPROM_state.EEPROMnew.mqttPassword));
    ...
    for (int i = 0; i < NR_RESERVED; i++)
      EEPROM_state.EEPROMnew.reserved[i] = 0;
  ...
  if (!strcmp(EEPROM_state.EEPROMnew.signature, EEPROM_SIGNATURE_OLD2)) {
    Serial_println(F("* [ESP] Old2 signature match, need to initiate outputMode, outputFilter, mqttInputByte4 and zero "
                     "more reserved bytes"));
    EEPROM_state.EEPROMnew.outputMode = INIT_OUTPUTMODE;
    EEPROM_state.EEPROMnew.outputFilter = INIT_OUTPUTFILTER;
    EEPROM_state.EEPROMnew.mqttInputByte4 = 0;
  ...
  if (!strcmp(EEPROM_state.EEPROMnew.signature, EEPROM_SIGNATURE_NEW)) {
    ...
    if (saveRebootReason != REBOOT_REASON_UNKNOWN) {
      if ((saveRebootReason == REBOOT_REASON_WIFIMAN) || (saveRebootReason == REBOOT_REASON_ETH)) {
        // avoid writing to EEPROM here, resulting in 2 writes per cycle, if ESP cycles/restarts due to WiFiMan or
        // ethernet problem
        delayedRebootReasonReset = 1;
      } else if (saveRebootReason == REBOOT_REASON_MQTT) {
        // avoid writing to EEPROM here, resulting in 2 writes per cycle, if ESP cycles/restarts due to MQTT not
        // connecting
...
#else
  {
    ...
    if (shouldSaveConfig) {
      ...
      strlcpy(EEPROM_state.EEPROMnew.mqttServer, WiFiManMqttServer.getValue(),
              sizeof(EEPROM_state.EEPROMnew.mqttServer));
      if ((!strlen(WiFiManMqttPort.getValue())) ||
          (sscanf(WiFiManMqttPort.getValue(), "%i", &EEPROM_state.EEPROMnew.mqttPort) != 1))
        EEPROM_state.EEPROMnew.mqttPort = MQTT_PORT;
      strlcpy(EEPROM_state.EEPROMnew.mqttUser, WiFiManMqttUser.getValue(), sizeof(EEPROM_state.EEPROMnew.mqttUser));
      strlcpy(EEPROM_state.EEPROMnew.mqttPassword, WiFiManMqttPassword.getValue(),
              sizeof(EEPROM_state.EEPROMnew.mqttPassword));
      if ((!strlen(WiFiMan_use_static_ip.getValue())) ||
          (sscanf(WiFiMan_use_static_ip.getValue(), "%i", &useStaticIP) != 1))
        useStaticIP = INIT_USE_STATIC_IP;
      EEPROM_state.EEPROMnew.useStaticIP = useStaticIP;
  ...
  mqttClient.setCredentials((EEPROM_state.EEPROMnew.mqttUser[0] == '\0') ? 0 : EEPROM_state.EEPROMnew.mqttUser,
                            (EEPROM_state.EEPROMnew.mqttPassword[0] == '\0') ? 0 : EEPROM_state.EEPROMnew.mqttPassword);
  // mqttClient.setWill(MQTT_WILL_TOPIC, MQTT_WILL_QOS, MQTT_WILL_RETAIN, MQTT_WILL_PAYLOAD); // TODO

  Serial_print(F("* [ESP] Clientname "));
  Serial_println(MQTT_CLIENTNAME);
  Serial_print(F("* [ESP] User "));
  Serial_println(EEPROM_state.EEPROMnew.mqttUser);
  // Serial_print(F("* [ESP] Password ")); Serial_println(EEPROM_state.EEPROMnew.mqttPassword);
  Serial_print(F("* [ESP] Server "));
  Serial_println(EEPROM_state.EEPROMnew.mqttServer);
  Serial_print(F("* [ESP] Port "));
  Serial_println(EEPROM_state.EEPROMnew.mqttPort);
...
#ifdef TELNET
  if (telnetSuccess) {
    Sprint_P(true, false, false, PSTR("* [ESP] Telnet running on %i.%i.%i.%i"), local_ip[0], local_ip[1], local_ip[2],
             local_ip[3]);
  } else {
  ...
  Sprint_P(
      true, true, true,
      PSTR("* [ESP] AVRISP: ATmega programming: avrdude -c avrisp -p atmega328p -P net:%i.%i.%i.%i:%i -t # or -U ..."),
      local_ip[0], local_ip[1], local_ip[2], local_ip[3], avrisp_port);
  // listen for avrdudes
  avrprog->begin();
#endif
  ...
  if (telnetSuccess) {
    Sprint_P(true, true, true, PSTR("* [ESP] TELNET: telnet %i.%i.%i.%i"), local_ip[0], local_ip[1], local_ip[2],
             local_ip[3]);
  } else {
...
void process_for_mqtt_json(byte *rb, int n) {
  ...
  if (!mqttConnected)
    Mqtt_disconnectSkippedPackets++;
  if (mqttConnected || MQTT_DISCONNECT_CONTINUE) {
    if (n == 3)
      bytes2keyvalue(rb[0], rb[2], EMPTY_PAYLOAD, rb + 3, mqtt_key, mqtt_value);
    for (byte i = 3; i < n; i++) {
      if (!--throttle) {
        ...
        if (kvrbyte == 9) {
          if (jsonTerm == 0) {
            ...
            if (outputMode & 0x0004)
              client_publish_mqtt(mqttJsondata, jsonString);
            if (outputMode & 0x0040)
              client_publish_telnet(mqttJsondata, jsonString);
            if (outputMode & 0x0400)
              client_publish_serial(mqttJsondata, jsonString);
            jsonStringp = 1;
          }
        } else {
          if (kvrbyte > 9) {
            kvrbyte = 0;
            Sprint_P(true, true, true, PSTR("* [ESP] Warning: kvrbyte > 9 Src 0x%02X Tp 0x%02X Byte %i"), rb[0], rb[2],
                     i);
          }
          for (byte j = 0; j < kvrbyte; j++) {
            ...
            if (kvr) {
              if (outputMode & 0x0002)
                client_publish_mqtt(mqtt_key, mqtt_value);
              if (outputMode & 0x0020)
                client_publish_telnet(mqtt_key, mqtt_value);
              if (outputMode & 0x0200)
                client_publish_serial(mqtt_key, mqtt_value);
              // don't add another parameter if remaining space is not enough for mqtt_key, mqtt_value, and ',' '"',
              // '"', ':', '}', and '\0'
...
// rh is packet size (without CRC byte)
{
  ...
  WB[rh] = crc;
  if (crc_gen)
    snprintf(pseudoWriteBuffer + 2 + timeStamp + (rh << 1), 3, "%02X", crc);
#if !((defined MQTT_INPUT_BINDATA) || (defined MQTT_INPUT_HEXDATA))
  if (outputMode & 0x0001)
    client_publish_mqtt(mqttHexdata, pseudoWriteBuffer);
#endif /* MQTT_INPUT_BINDATA || MQTT_INPUT_HEXDATA */
  if (outputMode & 0x0010)
    client_publish_telnet(mqttHexdata, pseudoWriteBuffer);
  if (outputMode & 0x0100)
    client_publish_serial(mqttHexdata, pseudoWriteBuffer);
#if !((defined MQTT_INPUT_BINDATA) || (defined MQTT_INPUT_HEXDATA))
  if ((outputMode & 0x0800) && (mqttConnected))
    mqttClient.publish((const char *)mqttBindata, MQTT_QOS, false, (const char *)WB, rh + 1);
#endif /* MQTT_INPUT_BINDATA || MQTT_INPUT_HEXDATA */
  if (outputMode & 0x0666)
    process_for_mqtt_json(WB, rh);
}
...
void loop() {
  ...
  while (milliInc >= 1000) {
    ...
    if (milliInc < 1000) {
      if ((throttleValue > 1) && (espUptime > (THROTTLE_VALUE - throttleValue + 1) * THROTTLE_STEP))
        throttleValue--;
      Sprint_P(true, true, false, PSTR("* [ESP] Uptime %li"), espUptime);
      // alternative, more info: Sprint_P(true, true, false, PSTR("* [ESP] %d RSSI %d heap %d maxfreeblocksize %d Uptime
      // %li disconnects %d WiFi %d ethernet %d status %d"), espUptime, WiFi.RSSI(), ESP.getFreeHeap(),
      // ESP.getMaxFreeBlockSize(), espUptime, Mqtt_disconnects, WiFi.isConnected(), eth.connected(), eth.status());
      ...
      if (!mqttClient.connected())
        Sprint_P(true, false, true, PSTR("* [ESP] MQTT is disconnected (%i s total %i s)"), Mqtt_disconnectTime,
                 Mqtt_disconnectTimeTotal);
      pseudo0D++;
      pseudo0E++;
      pseudo0F++;
  ...
  if (last_state != new_state) {
    ...
    case AVRISP_STATE_IDLE:
      Sprint_P(true, true, true, PSTR("* [ESP-AVRISP] now idle"));
      pinMode(RESET_PIN, INPUT);
      delay(200);
      ATmega_dummy_for_serial();
      break;
    case AVRISP_STATE_PENDING:
      Sprint_P(true, true, true, PSTR("* [ESP-AVRISP] connection pending"));
      pinMode(RESET_PIN, OUTPUT);
      break;
    case AVRISP_STATE_ACTIVE:
      Sprint_P(true, true, true, PSTR("* [ESP-AVRISP] programming mode"));
      pinMode(RESET_PIN, OUTPUT);
      break;
    }
  ...
  // if (WiFi.isConnected() || eth.connected()) { // TODO this does not work. eth.connected() becomes true after 4
  // minutes even if there is no ethernet cable attached
  // TODO and eth.connected() remains true even after disconnecting the ethernet cable (and perhaps also after 'D1' soft
  // ESP.reset()
  // ethernetConnected = ???;                     // TODO try to update ethernetConnected based on actual ethernet
  // status
  if (WiFi.isConnected() || ethernetConnected) { // TODO for now: use initial ethernet connection status instead
                                                 // (ethernet cable disconnect is thus not detected)
    if (!mqttClient.connected()) {
      if (espUptime > reconnectTime) {
        // Sprint_P(true, false, true, PSTR("* [ESP] MQTT Try to reconnect to MQTT server %s:%i (user %s/password %s)"),
        // EEPROM_state.EEPROMnew.mqttServer, EEPROM_state.EEPROMnew.mqttPort, EEPROM_state.EEPROMnew.mqttUser,
        // EEPROM_state.EEPROMnew.mqttPassword);
        Sprint_P(true, false, true, PSTR("* [ESP] MQTT Try to reconnect to MQTT server %s:%i (user %s/password *)"),
                 EEPROM_state.EEPROMnew.mqttServer, EEPROM_state.EEPROMnew.mqttPort, EEPROM_state.EEPROMnew.mqttUser);
        mqttClient.connect();
        delay(500);
        if (mqttClient.connected()) {
  ...
  if (!OTAbusy) {
    ...
    if (c >= 0) {
      if ((c == '\n') && (serial_rb < RB)) {
        ...
        *rb_buffer = '\0';
        ...
        if (ignoreremainder == 2) {
          ...
          if (readBuffer[0] == 'R') {
            ...
            if (rh == HB)
              Sprint_P(true, true, true, PSTR("* [MON] Buffer full, overflow not checked"));
            if ((rh > 1) || (rh == 1) && !crc_gen) {
              ...
              if ((!crc_gen) || (crc == readHex[rh])) {
                ...
                if (outputMode & 0x0100)
                  client_publish_serial(mqttHexdata, readBuffer);
#if !((defined MQTT_INPUT_BINDATA) || (defined MQTT_INPUT_HEXDATA))
                if ((outputMode & 0x0800) && (mqttConnected))
                  mqttClient.publish((const char *)mqttBindata, MQTT_QOS, false, (const char *)readHex, rh + 1);
#endif /* MQTT_INPUT_BINDATA || MQTT_INPUT_HEXDATA */
                if (outputMode & 0x0666)
                  process_for_mqtt_json(readHex, rh);
#ifdef PSEUDO_PACKETS
                if ((readHex[0] == 0x00) && (readHex[1] == 0x00) && (readHex[2] == 0x0D))
                  pseudo0D = 9; // Insert pseudo packet 40000D in output serial after 00000D
                if ((readHex[0] == 0x00) && (readHex[1] == 0x00) && (readHex[2] == 0x0F))
                  pseudo0F = 9; // Insert pseudo packet 40000F in output serial after 00000F
#endif
                ...
                Sprint_P(true, true, true,
                         PSTR("* [MON] Serial input buffer overrun or CRC error in R data:%s expected 0x%02X"),
                         readBuffer + 1, crc);
                if (ESP_serial_input_Errors_CRC < 0xFF)
                  ESP_serial_input_Errors_CRC++;
              }
            } else {
              Sprint_P(true, true, true, PSTR("* [MON] Not enough readable data in R line: ->%s<-"), readBuffer + 1);
              if (ESP_serial_input_Errors_Data_Short < 0xFF)
                ESP_serial_input_Errors_Data_Short++;
            }
        ...
        *(rb_buffer - 1) = '\0';
        if (c != '\n') {
          Sprint_P(true, true, true, PSTR("* [MON] Line too long, ignored, ignoring remainder: ->%s%c%c<-"), readBuffer,
                   lst, c);
          ignoreremainder = 1;
        } else {
    ...
    if (pseudo0D > 5) {
      ...
      for (int i = 19; i <= 22; i++)
        readHex[i] = 0x00; // reserved for future use
