...
float FN_f8_8(uint8_t *b)            { return (((int8_t)b[-1]) + 0.1 * (b[0] / 24)); }
...
// also modifies savehistoryp, savehistorylen, savehistoryend
  ...
  if (shi >= 0) {
    ...
    for (byte i = 5; i < n; i += 3) {
      ...
      byte paramval = rb[i];
      if ((paramnr != 0xFFFF) && (paramnr != 0xA2)) {Serial.print("* paramnr "); Serial.print(paramnr, HEX); Serial.print(" "); Serial.print(paramval, HEX); Serial.print(" "); Serial.println(paramcat);}
...
byte handleparam(char* key, char* value, byte* rb, uint8_t i) {
  int paramnr = (((uint16_t) rb[i-1]) << 8) | rb[i-2];
  byte paramval = rb[i];
// TODO check for known parameters...
 // if (/* (!outputunknown) || */ !newbytesval(rb, i, 0)) return 0;
