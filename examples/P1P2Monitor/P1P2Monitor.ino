...
void recvSerial()
{
  ...
  if (rxIndex != 0 && (millis() - startMillis >= SERIAL_TIMEOUT || RS[rxIndex - 1] == SERIAL_TERMINATOR)) {
    rs = rxIndex;
    rxIndex = 0;
    newRS = true;
  }
...
void processSerial() {
  ...
  while (P1P2Serial.packetavailable()) {
        ...
        if (CONTROL_ID && (FxAbsentCnt[CONTROL_ID & 0x01] == F0THRESHOLD) && (RB[1] == CONTROL_ID)) {
          ...
          switch (RB[2]) {
            ...
            case 0x35 : // in: 21 byte; out 21 byte; 3-byte parameters reply with FF
              ...
              if (setcoolingheatingrequest) {
                WB[w++] = setParam;
                WB[w++] = 0x00;
                WB[w++] = setcoolingheatingstatus;
                setcoolingheatingrequest = 0;
              }
    ...
    if (verbose) {
      ...
      if (delta < 1000) Serial.print(F("0")); else {
