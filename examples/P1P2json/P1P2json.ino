/* P1P2json: Monitor for reading Daikin/Rotex P1/P2 bus using P1P2Serial library, output in json format
...
void setup() {
  ...
  Serial.println(F("*"));
  Serial.println(F("*P1P2json v0.9.4"));
  Serial.println(F("*"));
...
void process_for_mqtt_json(byte* rb, int n) {
  ...
  char key[KEYLEN];
  for (byte i = 3; i < n; i++) {
    int kvrbyte = bytes2keyvalue(rb, i, 8, key, value);
