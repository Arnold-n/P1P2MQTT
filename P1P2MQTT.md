...
Ech topic includes \<xxx\>, which is the 4th byte of the IPv4 address, shown as a 3-character code (with leading zeroes if needed). This enables to easily derive its IPv4 address for telnet usage and to have multiple P1P2MQTT programs running. In the examples below \<xxx\> is 122.
...
In topic P1P2/P/\<xxx\>/\<X\>/\<SRC\> X is a single character:
  ...
  - A for data generated by software on ATmega or ESP8266
...
Further, supported but advised not to use, and not really needed (some may be removed in a future version):
